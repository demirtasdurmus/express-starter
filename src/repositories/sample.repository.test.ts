...
describe('Sample Repository', () => {
  ...
  describe('getSampleByName', () => {
    it('should return a sample when it exists', () => {
      const name = 'Unique Sample Name';
      const createdSample = createSample(name);
      const result = getSampleByName(name);

      expect(result).toEqual(createdSample);
    });

    it('should return null when sample does not exist', () => {
      const nonExistentName = 'Non-Existent Sample Name';
      const result = getSampleByName(nonExistentName);
